...
- The values within the list returned from `execute` must NEVER include promises
  or `FlaggedValue` objects such as errors
...
A value can be a repeatable async iterable created synchronously: consumers own
its asynchronous traversal, independently of this step's execution.
...
Steps that yield streamed results must return repeatable iterables. Each call to
`[Symbol.asyncIterator]()` (or `[Symbol.iterator]()` for synchronous iterables)
must return an independent iterator starting at the beginning. Gra*fast* can
traverse the same result for multiple fields or repeated list items; it does not
copy iterators or automatically materialize shared results.

Return an iterable that creates an iterator, rather than returning a single
async generator object:

```ts
return {
  async *[Symbol.asyncIterator]() {
    yield* await fetchRows();
  },
};
```

The producer owns fetching and any shared cache. Each iterator owns its position
and must release its resources on completion, errors, and cancellation. Creating
an iterator should register a consumer without fetching; begin fetching when
`next()` is called. No repeatability flag or marker is required.

List traversal belongs to execution of the list-item layer. Step results retain
their original iterable; Gra*fast* opens a separate iterator for each list
consumer and parent position. The layer prepares the initial items and executes
their child steps before synchronous output rendering. With an honoured
`@stream`, it retains the remaining iterator for incremental delivery; otherwise
it consumes the complete list. Arrays follow the same delivery rules, with an
optimized path for immediate arrays without promised items.

Streaming information passed to a step is a performance hint, not a promise of
incremental delivery. Producers must support either consumption pattern.
